        ...
        <div className="bg-gradient-to-r from-primary-900 to-primary-800 text-white p-6 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-green-300">Work faster with these shortcuts</p>
            </div>
          </div>
          <button
            onClick={onClose}
              ...
              <div key={idx}>
                ...
                <div className="space-y-2">
                  {category.items.map((shortcut, sIdx) => (
                    <div
                      key={sIdx}
                      className="flex items-center justify-between py-2 px-3 rounded-lg hover:bg-green-50 transition-colors"
                    >
                      <span className="text-sm text-primary-700">{shortcut.description}</span>
                      <div className="flex items-center space-x-1">
                        {shortcut.keys.map((key, kIdx) => (
                          <React.Fragment key={kIdx}>
                            ...
                            <kbd className="px-2 py-1 bg-green-100 border border-green-300 rounded text-xs font-mono text-primary-800 shadow-sm">
                              {key}
                            </kbd>
                          </React.Fragment>
                        ))}
