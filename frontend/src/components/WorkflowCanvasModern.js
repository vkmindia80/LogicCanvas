...
const nodeTypes = {
  // All node types use CustomNodeModern component with modern indigo/slate styling
  start: CustomNodeModern,
  end: CustomNodeModern,
  task: CustomNodeModern,
  form: CustomNodeModern,
  screen: CustomNodeModern,
  wait: CustomNodeModern,
  decision: CustomNodeModern,
  switch: CustomNodeModern,
  assignment: CustomNodeModern,
  loop_for_each: CustomNodeModern,
  loop_while: CustomNodeModern,
  loop_repeat: CustomNodeModern,
  approval: CustomNodeModern,
  create_record: CustomNodeModern,
  update_record: CustomNodeModern,
  delete_record: CustomNodeModern,
  lookup_record: CustomNodeModern,
  query_records: CustomNodeModern,
  get_record: CustomNodeModern,
  transform: CustomNodeModern,
  filter: CustomNodeModern,
  sort: CustomNodeModern,
  aggregate: CustomNodeModern,
  calculate: CustomNodeModern,
  parallel: CustomNodeModern,
  merge: CustomNodeModern,
  subprocess: CustomNodeModern,
  action: CustomNodeModern,
  api_call: CustomNodeModern,
  webhook: CustomNodeModern,
  email: CustomNodeModern,
  timer: CustomNodeModern,
  event: CustomNodeModern,
  error_handler: CustomNodeModern,
};
  ...
  return (
    <div className="flex h-screen bg-slate-50">
      ...
      <div className="flex-1 flex flex-col">
        {/* Top Toolbar - Modernized */}
        <div className="bg-gradient-to-r from-white to-slate-50/50 border-b border-slate-200 px-6 py-4 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center space-x-4 flex-1">
              <input
                ...
                className={`${modernInputStyles.base} text-xl font-bold max-w-md`}
                placeholder="Workflow Name"
                data-testid="workflow-name-input"
              />
              <div className="flex items-center space-x-2 text-sm">
                <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 text-indigo-700 rounded-lg font-medium ring-1 ring-inset ring-indigo-600/10 shadow-sm">
                  <div className="w-1.5 h-1.5 rounded-full bg-indigo-600 animate-pulse"></div>
                  {nodes.length} nodes
                </span>
                <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-purple-50 text-purple-700 rounded-lg font-medium ring-1 ring-inset ring-purple-600/10 shadow-sm">
                  <div className="w-1.5 h-1.5 rounded-full bg-purple-600 animate-pulse"></div>
                  {edges.length} connections
                </span>
              </div>
            </div>
            {/* Modern Save Status Badge */}
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-xl text-sm font-medium shadow-sm">
              {isSaving ? (
                <>
                ...
                <>
                  ...
                  <span className="text-slate-700">{formatLastSaved()}</span>
                </>
              ) : (
                <>
        ...
        <div className="flex-1" ref={reactFlowWrapper}>
          <ReactFlow
            ...
            nodeTypes={nodeTypes}
            edgeTypes={{ deletable: DeletableEdgeModern }}
            fitView
      ...
      {selectedNode && !showExecutionPanel && !showTriggerConfig && (
        <div className="fixed md:relative right-0 top-0 w-full md:w-80 lg:w-96 h-screen bg-gradient-to-b from-white to-slate-50 border-l border-slate-200 shadow-2xl md:shadow-lg p-4 overflow-y-auto flex-shrink-0 z-40">
          <NodeEditorModern
            node={selectedNode}
            onUpdate={updateNode}
            onDelete={deleteNode}
