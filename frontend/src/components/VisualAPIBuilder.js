          ...
          <div className="flex items-start space-x-2 mt-2 text-xs">
            ...
            <button
              onClick={() => {
                // Format JSON
                try {
                  const formatted = JSON.stringify(JSON.parse(typeof config.body === 'string' ? config.body : JSON.stringify(config.body)), null, 2);
                  updateConfig({ body: JSON.parse(formatted) });
                } catch (e) {
                  // Invalid JSON, ignore
                }
              }}
