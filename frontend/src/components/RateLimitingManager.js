...
const RateLimitingManager = ({ onClose, connectorId = null }) => {
  ...
  const getUsageColor = (percentage) => {
    if (percentage >= 90) return 'text-gold-600 bg-gold-100';
  ...
  const getCircuitBreakerStatus = (state) => {
    switch (state) {
      ...
      case 'open':
        return { icon: AlertTriangle, color: 'text-gold-600', label: 'Open (Failing)' };
      case 'half_open':
        return { icon: Activity, color: 'text-gold-600', label: 'Testing' };
      default:
        return { icon: Info, color: 'text-primary-600', label: 'Unknown' };
    }
  };
                      ...
                      <div className="space-y-3">
                          ...
                          <div className="w-full bg-green-200 rounded-full h-3">
                            <div
                              className={`h-3 rounded-full transition-all ${
                                usagePercentage >= 90 ? 'bg-gold-500' : 
                        ...
                        <div className="grid grid-cols-3 gap-2 text-sm">
                          <div className="bg-white rounded-lg p-2">
                            <div className="text-green-500 text-xs">Time Window</div>
                            <div className="font-semibold text-primary-900">
                              {rateLimitConfig.time_window_seconds}s
                            </div>
                          </div>
                          <div className="bg-white rounded-lg p-2">
                            <div className="text-green-500 text-xs">Burst Size</div>
                            <div className="font-semibold text-primary-900">
                              {rateLimitConfig.burst_size}
                            </div>
                          </div>
                          <div className="bg-white rounded-lg p-2">
                            <div className="text-green-500 text-xs">Strategy</div>
                            <div className="font-semibold text-primary-900 capitalize text-xs">
